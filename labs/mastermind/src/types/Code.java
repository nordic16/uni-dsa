...
public class Code{
	...
	public int[] howManyCorrect(Code other) {
		...
		int a = 0, b = 0, c = 0; // c controls foundColors position.
		
		for (int i = 0; i < other.getLength(); i++) {
			
			if (this.code[i] == other.code[i]) {
				if (!isPresent(foundColors, this.code[i])) {
					foundColors[c++] = this.code[i];
				}
				a++;
			
			} else if (!isPresent(foundColors, this.code[i]) && isPresent(other.code, this.code[i])) {  // if colour isn't present yet, increase b.
				foundColors[c++] = this.code[i];
				b++;
			}
		}	

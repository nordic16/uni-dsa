...
public class Mastermind {
	...
	public String toString() { // might not be the best solution but oh well.
		...
		builder.insert(0, "Number of Trials = " + this.numTrials +  EOL + "Score = " + this.score + EOL);
		
		for (Code c : this.trials) {
			if (c == null) break;
			
			int[] results = c.howManyCorrect(this.genCode);
			builder.append(c.toString() + "    " + results[0] + " " + results[1] + EOL);
		}		
	...
	private int getTrialIndex(Code code) {
		...
		// finds index of last trial.
		for (index = 0; index < this.trials.length - 1; index++) {
			if (this.trials[index + 1] == null) break;
		}
		
		return code.equalCodes(this.trials[index % 10]) ? index % 10 : (index + 1) % 10;
	}
